...
class XUIClient:
    ...
    def _build_client_config(self, inbound: Dict[str, Any], target_client: Dict[str, Any]) -> Dict[str, Any]:
        """
        Собирает клиентский конфиг из inbound и client записи.
        """
    ...
    async def get_client_config_for_inbound(self, email: str, inbound_id: int) -> Optional[Dict[str, Any]]:
        """
        Возвращает конфиг клиента в конкретном inbound.
        """
        try:
            inbounds = await self.get_inbounds()
            for inbound in inbounds:
                ...
                for client in clients:
                    if client.get("email") == email:
                        return self._build_client_config(inbound, client)
        except Exception as e:
            logger.error(f"Error getting client config for inbound {inbound_id}: {e}")
        return None
    ...
    async def get_client_config(self, email: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for inbound in inbounds:
                ...
                if target_client:
                    return self._build_client_config(inbound, target_client)
        except Exception as e:
