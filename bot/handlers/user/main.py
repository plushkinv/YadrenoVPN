...
async def activate_trial_subscription(callback: CallbackQuery, state: FSMContext):
    """Активирует пробную подписку и сразу выдаёт subscription-ссылку."""
    from database.requests import (
    ...
    await finalize_payment_ui(
        ...
        {"order_id": order_id, "vpn_key_id": key_id},
        user_id=callback.from_user.id
    )
...
async def show_my_keys(telegram_id: int, send_function):
    ...
    if not keys:
        await send_function(
            ...
            "Нажмите «Купить подписку» на главной, чтобы приобрести доступ! 🚀",
...
async def buy_key_handler(callback: CallbackQuery):
    """Страница «Купить подписку» с условиями и способами оплаты."""
    from database.requests import (
        is_crypto_configured, is_stars_enabled, is_cards_enabled, get_setting,
...
• 1 подписка = 1 устройство (одновременное подключение)
• Лимит трафика: до 1 ТБ в месяц (сброс каждые 30 дней)

⚠️ *Важно знать:*
• Средства не возвращаются — услуга считается оказанной в момент получения подписки
...
_Приобретая подписку, вы соглашаетесь с этими условиями._
