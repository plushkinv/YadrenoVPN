...
Продлите подписку, чтобы сохранить доступ к VPN без перерыва!'''),
        ...
        ('help_page_text', (
            ...
            "*Как это работает:*\n"
            "1\\. Купите подписку через раздел «Купить подписку»\n\n"
...
def migration_7(conn: sqlite3.Connection) -> None:
    """
    Миграция v7: Единые subscription-ссылки для ключа на нескольких серверах.

    Изменения:
    - Добавляет колонку subscription_token в vpn_keys
    - Добавляет таблицу vpn_key_nodes для хранения конфигов по серверам
    - Добавляет настройки subscription_base_url/subscription_bind_host/subscription_bind_port
    """
    logger.info("Применение миграции v7 (Unified subscription)...")

    try:
        conn.execute("ALTER TABLE vpn_keys ADD COLUMN subscription_token TEXT")
        logger.info("Колонка subscription_token добавлена в таблицу vpn_keys")
    except sqlite3.OperationalError as e:
        if "duplicate column name" in str(e):
            logger.info("Колонка subscription_token уже существует")
        else:
            raise

    conn.execute("""
        CREATE UNIQUE INDEX IF NOT EXISTS idx_vpn_keys_subscription_token
        ON vpn_keys(subscription_token)
    """)

    conn.execute("""
        CREATE TABLE IF NOT EXISTS vpn_key_nodes (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            vpn_key_id INTEGER NOT NULL,
    ...
    for key, value in subscription_settings:
        conn.execute(
            "INSERT OR IGNORE INTO settings (key, value) VALUES (?, ?)",
            (key, value)
        )
