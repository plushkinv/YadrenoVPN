...
def set_vpn_key_subscription_token(key_id: int, token: str) -> bool:
    """
    Сохраняет subscription token для ключа.
    """
    with get_db() as conn:
        cursor = conn.execute("""
            UPDATE vpn_keys
...
def get_vpn_key_subscription_token(key_id: int) -> Optional[str]:
    """
    Возвращает subscription token ключа.
    """
    with get_db() as conn:
        cursor = conn.execute(
            "SELECT subscription_token FROM vpn_keys WHERE id = ?",
            (key_id,)
        )
        row = cursor.fetchone()
        if not row:
...
def get_vpn_key_nodes(vpn_key_id: int) -> List[Dict[str, Any]]:
    """
    Возвращает серверные ноды подписки для ключа.
    """
    with get_db() as conn:
        cursor = conn.execute("""
            SELECT
            ...
            FROM vpn_key_nodes n
            LEFT JOIN servers s ON s.id = n.server_id
            WHERE n.vpn_key_id = ?
            ORDER BY n.server_id, n.panel_inbound_id
        """, (vpn_key_id,))
        return [dict(row) for row in cursor.fetchall()]


def replace_vpn_key_nodes(vpn_key_id: int, nodes: List[Dict[str, Any]]) -> None:
    """
    Полностью заменяет список серверных нод подписки для ключа.
    """
    with get_db() as conn:
        conn.execute("DELETE FROM vpn_key_nodes WHERE vpn_key_id = ?", (vpn_key_id,))
        for node in nodes:
            conn.execute("""
                INSERT INTO vpn_key_nodes
                (vpn_key_id, server_id, panel_inbound_id, panel_email, client_uuid, access_link)
                VALUES (?, ?, ?, ?, ?, ?)
            """, (
                vpn_key_id,
                node['server_id'],
                node['panel_inbound_id'],
                node['panel_email'],
                node['client_uuid'],
                node['access_link'],
            ))
...
def get_subscription_links_by_token(token: str) -> List[str]:
    """
    Возвращает список ссылок подписки по subscription token.
    Пустой список, если токен не найден или ключ истёк.
    """
    with get_db() as conn:
        ...
        cursor = conn.execute("""
            ...
            FROM vpn_key_nodes
            WHERE vpn_key_id = ?
            ORDER BY server_id, panel_inbound_id
        """, (key_row['id'],))
        return [row['access_link'] for row in cursor.fetchall()]
